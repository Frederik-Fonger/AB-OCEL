...
def calculate_single_event(context, binding, object_types, ocpn, counter, lock):
    ...
    while not index == len(q):
        ...
        if index > 70000:
            ...
            with open("MP_output.txt", "a") as f:
                f.write("Aborting: Timeout single event. Length: " + str(len(q)) +  " | Count: " + str(counter.value) + '\n')
            break
    ...
    # =============================================================================
    print("One Calculation Done. Lenght was: " + str(len(q)))
    with lock:
        counter.value += 1
        print(f" Count: {str(counter.value)}")
    with open("MP_output.txt", "a") as f:
        f.write("One Calculation Done. Lenght was: " + str(len(q)) +  " | Count: " + str(counter.value) + '\n')
    return result, times


def enabled_model_activities_multiprocessing_old(contexts, bindings, ocpn, object_types):
    pool = ThreadPool(5)
...
def enabled_model_activities_multiprocessing(contexts, bindings, ocpn, object_types, output_folder):
    ...
    with multiprocessing.Pool(processes=6) as pool:
        context_list = [contexts[i] for i in contexts.keys()]
        binding_list = [bindings[i] for i in contexts.keys()]
        print("len context_list: " + str(len(context_list)))
        with open(output_folder + "MP_output.txt", "w") as f:
            f.write("len context_list: " + str(len(context_list)) + '\n')
        

        result = pool.starmap(
            calculate_single_event,
            zip(context_list, binding_list, itertools.repeat(object_types), itertools.repeat(ocpn), itertools.repeat(counter),
...
def calculate_precision_and_fitness(ocel, context_mapping, en_l, en_m):
    ...
    fit = []
    skipped = 0
    for index, row in tqdm(ocel.log.iterrows()):
        e_id = row["event_id"]
